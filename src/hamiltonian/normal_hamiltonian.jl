...
function p_copy(h::Hamiltonian)
    Hamiltonian(h.op, zeros(eltype(h.u_cache), size(h.u_cache)))
end

function p_copy(h::HamiltonianSparse)
    HamiltonianSparse(h.op, spzeros(eltype(h.u_cache), size(h.u_cache)...))
end
